            ...
            {["Home", "About", "Education", "Project", "Resume", "Contact"].map(
              (section) => (
                <Link
                  ...
                  href={`#${section}`}
                  scroll={true}
                  className={`block px-3 py-2 rounded-md text-lg font-medium ${isActive(
                    `#${section}`
                  )}`}
                  onClick={() => setCurrentPath(`#${section}`)}>
                  {section === "Home"
        ...
        {showIcons && (
          <div className='flex flex-col space-y-2'>
            <button
              ...
              {darkMode ? (
                <BsFillSunFill size={24} className='text-yellow-500' />
              ) : (
                <BsMoonStarsFill size={20} />
              )}
            </button>
            <a
              href='https://id.linkedin.com/in/nur-azani-bayu-rezki-08369a219'
              target='_blank'
            ...
            <a
              href='https://github.com/bayurz11'
              target='_blank'
              rel='noopener noreferrer'
              className='p-3 bg-gray-800 text-white rounded-full shadow-lg hover:bg-gray-700 focus:outline-none'>
              <FaGithub size={24} />
